            ...
            {tokens.map((token, index) => (
              <li key={token.symbol}>
                <input
                  ...
                  ref={form.register}
                />
                <label htmlFor={`assets[${index}]`}>{token.symbol} ({token.name})</label>
              </li>
            ))}
          </ul>

          <h3>Disclaimer for the use of the Melon Protocol</h3>
          <p>IMPORTANT NOTE: PLEASE READ THE FULL VERSION OF THIS DISCLAIMER CAREFULLY BEFORE USING THE MELON PROTOCOL.</p>
          <p>
            YOUR USE OF THE MELON PROTOCOL AND/OR THE SOFTWARE MAY BE SUBJECT TO THE FINANCIAL LAWS AND REGULATIONS OF VARIOUS
